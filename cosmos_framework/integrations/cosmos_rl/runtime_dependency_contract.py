...
_OLD_CONV_GUARD = 'if self.enable_linear and (is_torch_equal("2.9.0") or is_torch_equal("2.9.1")):'
...
_QWEN_FORCE_ANCHOR = 'FORCE_QWENVL_VIDEO_READER = os.getenv("FORCE_QWENVL_VIDEO_READER", None)\n'
...
def _ensure_forced_video_reader(video_reader_backend: str) -> None:
    ...
    register_pynv_video_reader(
        cache_size=int(os.getenv("COSMOS_PYNV_VIDEO_CACHE_SIZE", "0")),
        decoder_cache_size=int(os.getenv("COSMOS_PYNV_DECODER_CACHE_SIZE", "4")),
        video_override_map=os.getenv("COSMOS_PYNV_VIDEO_OVERRIDE_MAP") or None,
        strict=_env_flag("COSMOS_PYNV_VIDEO_STRICT", True),
    )
...
_QWEN_NEW_FALLBACK = """        except Exception as e:
            if video_reader_backend == "pynvvideocodec" or _env_flag(
                "COSMOS_PYNV_VIDEO_STRICT", False
            ):
                raise RuntimeError(
...
def repair_vllm_conv3d_source(source: str) -> tuple[str, bool]:
    ...
    if source.count(_OLD_CONV_IMPORT) != 1 or source.count(_OLD_CONV_GUARD) != 1:
        raise RuntimeError("Unrecognized vLLM Conv3D implementation; refusing blind patch")
    return (
        source.replace(_OLD_CONV_IMPORT, _NEW_CONV_IMPORT).replace(_OLD_CONV_GUARD, _NEW_CONV_GUARD),
        True,
    )
...
def repair_qwen_pynv_worker_source(source: str) -> tuple[str, bool]:
    ...
    worker_markers = (
        ...
        "_ensure_forced_video_reader(video_reader_backend)",
        'os.getenv("COSMOS_PYNV_DECODER_CACHE_SIZE", "4")',
        "strict GPU video decoding failed; CPU fallback is disabled",
    )
    pixel_markers = (
        "from cosmos_framework.inference.reasoner.video_pixel_bounds import normalize_video_pixel_bounds",
    ...
    if any(worker_markers_present) and not all(worker_markers_present):
        raise RuntimeError("Unrecognized qwen-vl-utils video implementation; refusing blind patch")
    if any(pixel_markers_present) and not all(pixel_markers_present):
        raise RuntimeError("Unrecognized qwen-vl-utils pixel-bound contract; refusing blind patch")
    ...
    if not all(pixel_markers_present):
        if repaired.count(_QWEN_FETCH_ANCHOR) != 1:
            raise RuntimeError("Unrecognized qwen-vl-utils fetch_video implementation; refusing blind patch")
        repaired = repaired.replace(
            _QWEN_FETCH_ANCHOR,
...
def verify_qwen_pynv_worker() -> None:
    ...
    if missing:
        raise RuntimeError("qwen-vl-utils strict GPU worker contract is missing: " + ", ".join(missing))
...
def repair_vllm_conv3d() -> None:
    ...
    if torch_version < Version("2.9"):
        print(f"vLLM Conv3D compatibility guard not required for PyTorch {torch_version}")
        return
...
def verify_deepep() -> None:
    ...
    if missing:
        raise RuntimeError("DeepEP extension is missing required internode symbols: " + ", ".join(missing))
