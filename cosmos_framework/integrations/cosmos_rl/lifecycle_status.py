...
"""Durable terminal Cosmos lifecycle status writes.

Framework's shared logger remains responsible for progress events. Terminal
...
def is_lifecycle_status_owner(environ: Mapping[str, str] | None = None) -> bool:
    ...
    Policy workers emit progress through ``CosmosStatusLogger``. The controller
...
def append_terminal_status(filename: str, status: str, message: str) -> None:
    """Append and verify one terminal Cosmos JSONL record."""
    if status not in _TERMINAL_STATUSES:
        raise ValueError(f"terminal Cosmos status must be one of {_TERMINAL_STATUSES}, got {status!r}")
    ...
    if final_record.get("status") != status:
        raise RuntimeError(
            f"terminal Cosmos status verification failed: expected {status}, found {final_record.get('status')!r}"
        )
