...
def _load_cache_class():
    source_path = Path(__file__).parents[2] / "cosmos_framework/integrations/cosmos_rl/feature_cache.py"
    tree = ast.parse(source_path.read_text(encoding="utf-8"))
    node = next(
        item for item in tree.body if isinstance(item, ast.ClassDef) and item.name == "_ValidationVideoFeatureCache"
    )
...
def _load_video_key_extractor():
    source_path = Path(__file__).parents[2] / "cosmos_framework/integrations/cosmos_rl/packer.py"
    tree = ast.parse(source_path.read_text(encoding="utf-8"))
    class_node = next(
        item for item in tree.body if isinstance(item, ast.ClassDef) and item.name == "HFVLMDataPackerMethods"
    )
    node = next(
        item
        for item in class_node.body
        if isinstance(item, ast.FunctionDef) and item.name == "_extract_video_cache_keys"
    )
    node.decorator_list = []
    module = ast.fix_missing_locations(ast.Module(body=[node], type_ignores=[]))
...
def test_validation_cache_uses_grid_in_identity_and_obeys_lru_capacity():
    ...
    cache.get_or_encode(["same"], torch.ones(4, 1), torch.tensor([[1, 2, 2]]), encode, 1)
    cache.get_or_encode(["same"], torch.ones(8, 1), torch.tensor([[1, 2, 4]]), encode, 1)
...
def test_validation_cache_keeps_fsdp_encoder_calls_rank_synchronous(monkeypatch):
    ...
    def encode(pixel_values, grids):
        ...
        split_sizes = grids.prod(-1).tolist()
        main = tuple(chunk.sum().reshape(1, 1) for chunk in torch.split(pixel_values, split_sizes))
        return main, []
