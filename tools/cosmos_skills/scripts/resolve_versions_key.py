...
# install, or anywhere else the tree is unpacked. $COSMOS_SKILLS_ROOT overrides it
...
DEFAULT_SKILL_BANK = Path(os.environ.get("COSMOS_SKILLS_ROOT") or _BUNDLED_SKILL_BANK)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to the packaged Cosmos workflow bundle (defaults to $COSMOS_SKILLS_ROOT).",
    )
...
def resolve(versions_path: Path, dotted_key: str) -> str:
    ...
    if not isinstance(cursor, str) or not cursor.strip():
        raise ValueError(f"key '{dotted_key}' did not resolve to a non-empty string in {versions_path}")
    return cursor.strip()
