...
SKILL = ROOT / "skills" / "models" / "cosmos3-reasoner"
...
def test_ready_plan_emits_reusable_backend_isolated_execution_bundle(
    ...
    schema = json.loads(
        (ROOT / "skills" / "core" / "cosmos-artifacts" / "references" / "spec_bundle.schema.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    generated = generator.apply_cosmos_evaluate_resolution_metadata(
        {
            ...
            "properties": {
                ...
                "vision": {"properties": {"nframes": {"default": 8, "enum": [4, 8, 16]}}},
                "generation": {
                    "properties": {
                ...
                "metrics": {"properties": {"names": {"default": ["bleu", "rouge"]}}},
            },
        }
    )
