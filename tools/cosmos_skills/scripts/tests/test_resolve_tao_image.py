...
COSMOS_SKILL = ROOT / "skills" / "models" / "cosmos3-reasoner"
...
def test_model_listing_reads_cosmos_backend_images_from_skill_info():
    capabilities = list_tao_models.load_backend_capabilities(ROOT, "cosmos3-reasoner")

    implementations = capabilities["implementations"]
    assert implementations["cosmos-rl"]["container_image"] == COSMOS_RL_IMAGE
    assert implementations["cosmos-framework"]["container_image"] == COSMOS_FRAMEWORK_IMAGE
