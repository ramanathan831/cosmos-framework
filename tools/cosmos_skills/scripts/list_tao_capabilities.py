...
"""Summarize Cosmos workflow bundle capabilities for plugin capability answers."""
...
def scan_platforms(skill_bank: Path) -> list[dict[str, Any]]:
    ...
    (skills/platform/cosmos-run-on-*/SKILL.md) instead of a central manifest. Any
    platform skill present in the tree self-registers — no registration step."""
    root = skill_bank.expanduser() / "skills" / "platform"
    platforms = []
    for skill_md in sorted(root.glob("cosmos-run-on-*/SKILL.md")):
        short = skill_md.parent.name[len("cosmos-run-on-") :]
...
def skill_capability_record(skill_bank: Path, skill_md: Path) -> dict[str, Any]:
    ...
    description = clean_description(
        str(config.get("description") or metadata.get("description") or f"Use the {skill_dir.name} skill.")
    )
...
def application_capabilities(skill_bank: Path) -> list[dict[str, Any]]:
    ...
    for skill_md in sorted(application_root.glob("*/SKILL.md")):
        ...
        record["files"] = sorted(item.name for item in skill_md.parent.iterdir() if item.is_file())
...
def build_capabilities(skill_bank: Path) -> dict[str, Any]:
    ...
    training_models = [item["model"] for item in model_records if "train" in item.get("actions", [])]
    full_finetune_models = [
        item["model"] for item in model_records if all(action in item.get("actions", []) for action in FLOW_ACTIONS)
    ]
    ...
    return {
        ...
        "platforms": {
            "source": "installed platform skills (skills/platform/cosmos-run-on-*)",
...
def format_capabilities_text(data: dict[str, Any]) -> str:
    ...
    for data_skill in data["data_workflows"]:
        action_text = f" Actions: {csv(data_skill['actions'])}." if data_skill["actions"] else ""
        lines.append(f"- {data_skill['name']}: {data_skill['capability']}{action_text}")
    ...
    lines.extend(
        [
            ...
            "- Full train/evaluate/inference/export/TensorRT flow: " + csv(full_flow_models),
    ...
    if automl["unsupported"]:
        lines.append(
            ...
            + "; ".join(f"{item['model']} ({item['reason']})" for item in automl["unsupported"])
        )
    else:
    ...
    lines.extend(
        [
            ...
            "KPI, and execution platform. I can select the matching Cosmos workflow, "
