...
def build_automl_support(skill_bank: Path, action_filter: str = "") -> dict[str, Any]:
    ...
    for model in sorted(model_records):
                ...
                supported_actions.append(
                    {
                        ...
                        "spec_template": (action_manifest.get("spec_template") or metadata.get("spec_template")),
                        "automl_default_parameters": (
        ...
        if action_filter and not candidate_actions:
            ...
            reason = "; ".join(f"{item['action']}: {item['reason']}" for item in unsupported_actions)
        else:
...
def format_all_models_text(data: dict[str, Any]) -> str:
    ...
    for item in data["models"]:
        train_schema = (
            "train schema valid" if item["has_train_schema"] else f"train schema {item['train_schema_status']}"
        )
        lines.append(
        ...
        for backend, details in implementations.items():
            supported = [action for action, config in details.get("actions", {}).items() if config.get("supported")]
...
def format_automl_text(data: dict[str, Any]) -> str:
    ...
    if data["supported"]:
        for item in data["supported"]:
            ...
            lines.append(f"- {item['model']} ({item['network_arch']}): automl_enabled=true; actions: {actions_text}")
    else:
        lines.append("- None")
