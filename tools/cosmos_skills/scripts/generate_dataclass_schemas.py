...
"""Maintenance tool: regenerate packaged Cosmos action schemas from installed runtimes.
...
This maintenance command requires the selected runtime configuration packages.
Normal bundle use consumes the checked-in schemas and does not run this generator.
"""
...
def filter_schema(schema: dict[str, Any], valid_actions: set[str], current_action: str) -> dict[str, Any]:
    ...
    schema["default"] = {key: value for key, value in schema.get("default", {}).items() if key in allowed_keys}
    return schema
...
def apply_cosmos_evaluate_resolution_metadata(schema: dict[str, Any]) -> dict[str, Any]:
    ...
    schema["x_tao_resolution"] = {
        ...
        "infer_from_training_plan": [
            ...
            "vision.video_start",
            "vision.video_end",
            "vision.resized_height",
            "vision.resized_width",
            "vision.min_pixels",
            "vision.total_pixels",
        ],
