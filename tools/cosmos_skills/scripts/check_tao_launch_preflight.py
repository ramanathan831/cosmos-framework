...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--min-gpu-memory-gb",
        type=float,
        help=("Fail local Docker preflight if any visible GPU has less than this much memory."),
    )
    parser.add_argument(
        "--low-vram-threshold-gb",
        type=float,
        default=DEFAULT_LOW_VRAM_THRESHOLD_GB,
        help=("Print a low-VRAM warning for local Docker GPUs below this memory threshold. Default: %(default)s"),
    )
...
def resolve_platform(skill_bank: Path, requested: str) -> dict[str, Any]:
    ...
    skills/platform/cosmos-run-on-<name>/references/skill_info.yaml — with no central
    ...
    info = skill_bank.expanduser() / "skills" / "platform" / f"cosmos-run-on-{name}" / "references" / "skill_info.yaml"
    if info.is_file():
    ...
    print(
        ...
        f"(skills/platform/cosmos-run-on-{name}/) — treating it as a self-describing, "
...
def parse_required_fields(values: list[str]) -> dict[str, list[str]]:
    ...
    for value in values:
        if "=" not in value:
            raise SystemExit("--json-required-field must use LABEL=FIELD[,FIELD...] syntax")
        ...
        if not label or not parsed_fields:
            raise SystemExit("--json-required-field must include a label and at least one field")
...
def normalize_gpu_arch(value: str) -> str:
    ...
    raise SystemExit(f"Unsupported GPU architecture format: {value}. Use sm_90, sm_103a, 90, or 9.0.")
...
def gpu_arch_is_supported(target: str, supported: set[str]) -> bool:
    ...
    return any(re.sub(r"[af]$", "", normalize_gpu_arch(candidate)) == target_family for candidate in supported)
...
def parse_gpu_arch_allowlists(values: list[str]) -> dict[str, set[str]]:
    ...
    for value in values:
        ...
        label = label.strip()
        arches = {normalize_gpu_arch(arch) for arch in raw_arches.split(",") if arch.strip()}
        if not label or not arches:
            raise SystemExit("--gpu-arch-allowlist must include a label and at least one SM value")
...
def parse_effective_batch_limits(values: list[str]) -> dict[str, list[tuple[int, int]]]:
    ...
    for value in values:
        if "=" not in value:
            raise SystemExit("--effective-batch-limit must use LABEL=BATCH_SIZE,SHARD_COUNT syntax")
        label, raw_values = value.split("=", 1)
        parts = [part.strip() for part in raw_values.split(",") if part.strip()]
        if len(parts) != 2:
            raise SystemExit("--effective-batch-limit must include exactly BATCH_SIZE,SHARD_COUNT")
        try:
            batch_size = int(parts[0])
            shard_count = int(parts[1])
        except ValueError as exc:
            raise SystemExit("--effective-batch-limit values must be integers") from exc
        if batch_size <= 0 or shard_count <= 0:
            raise SystemExit("--effective-batch-limit values must be positive integers")
...
def parse_min_free_disk_gb(values: list[str]) -> dict[str, float]:
    ...
    for value in values:
        ...
        if not label or gib <= 0:
            raise SystemExit("--min-free-disk-gb must include a label and a positive GIB value")
...
def check_free_disk_space(
    ...
    *,
    ...
    for label, required_gib in requirements.items():
        ...
        if path is None:
            print(f"Free-disk check failed: {label}={raw_path} is not a local filesystem path")
            ok = False
            continue
        try:
        ...
        if free_gib < required_gib:
            print(f"Free-disk check failed: {label}={path}, free={free_gib:.1f}GiB < required={required_gib:g}GiB")
            ok = False
        else:
            print(f"Free-disk OK: {label}={path}, free={free_gib:.1f}GiB, required={required_gib:g}GiB")
    return ok
...
def detect_local_gpu_memory_gb() -> list[float]:
    ...
    if not ok:
        return []
    return [float(gpu["memory_mib"]) / 1024.0 for gpu in gpus if gpu.get("memory_mib") is not None]
    ...
    for label, allowed in allowlists.items():
        ...
        for arch in target_arches:
            if not gpu_arch_is_supported(arch, allowed):
                print(f"GPU architecture unsupported for {label}: target={arch}, allowed={','.join(sorted(allowed))}")
                label_ok = False
                ok = False
        if label_ok:
            print(
...
def check_gpu_resources(
    ...
    if target_memory_gb:
        ...
        if len(detected_memory) < target_count:
            print(f"GPU resource check failed: detected fewer GPUs than --target-gpu-count={target_count}")
            return False
...
def filter_target_gpus(gpus: list[dict[str, Any]], target_indices: list[str]) -> tuple[bool, list[dict[str, Any]]]:
    if not target_indices:
        return True, gpus
    by_index = {str(gpu.get("index")): gpu for gpu in gpus}
    missing = [index for index in target_indices if index not in by_index]
    if missing:
        print("Target GPU selection failed: missing index(es)=" + ",".join(missing))
        return False, []
...
def ssh_command(host: str, remote_command: str) -> list[str]:
    ...
    if key_path:
        command.extend(["-i", str(Path(key_path).expanduser()), "-o", "IdentitiesOnly=yes"])
...
def load_json_payload(path: str) -> Any:
    ...
    if path.startswith("s3://"):
        aws = shutil.which("aws")
        if not aws:
            raise RuntimeError("aws CLI not found. After user approval, install it with: python -m pip install awscli")
        missing = [key for key in ("ACCESS_KEY", "SECRET_KEY") if not os.environ.get(key)]
        if missing:
...
def check_effective_batch_limits(
    ...
    if not limits:
        return True
    if skip_access:
        print("Effective batch limits present; skipped annotation record-count checks.")
        return True
...
def parse_gpu_query_output(stdout: str, has_compute_cap: bool, *, has_uuid: bool = False) -> list[dict[str, Any]]:
    ...
    for row in csv.reader(stdout.splitlines()):
        ...
        compute_cap = row[compute_cap_index].strip() if has_compute_cap and len(row) > compute_cap_index else ""
        gpus.append(
            {
                "index": row[0].strip(),
...
def filter_cuda_visible_gpus(
    ...
    if raw_visibility is None:
        ...
        print(f"CUDA-visible GPU comparison: physical={physical_count}, visible={len(visible)}, source=unrestricted")
        return True, visible

    raw = raw_visibility.strip()
    if not raw or raw == "-1":
        print(f"CUDA-visible GPU comparison: physical={physical_count}, visible=0, source=CUDA_VISIBLE_DEVICES")
        return False, []
    ...
    for token in tokens:
        ...
        if match is None and token.upper().startswith("GPU-"):
            candidates = [gpu for gpu in gpus if str(gpu.get("uuid", "")).upper().startswith(token.upper())]
            if len(candidates) == 1:
                match = candidates[0]
        if match is None:
            print(f"CUDA_VISIBLE_DEVICES cannot be resolved against host inventory: token={token!r}")
            return False, []
        host_id = str(match.get("index"))
        if host_id in selected_host_ids:
            print(f"CUDA_VISIBLE_DEVICES selects the same host GPU more than once: index={host_id}")
            return False, []
        selected_host_ids.add(host_id)
        selected.append(dict(match, visible_index=str(len(selected))))

    print(
        f"CUDA-visible GPU comparison: physical={physical_count}, visible={len(selected)}, source=CUDA_VISIBLE_DEVICES"
    )
    return True, selected
...
def query_host_gpus(*, print_result: bool = True) -> tuple[bool, list[dict[str, Any]]]:
    ...
    if not gpus:
        ...
        return False, []
    visibility_ok, visible_gpus = filter_cuda_visible_gpus(gpus, os.environ.get("CUDA_VISIBLE_DEVICES"))
...
def check_slurm(
    ...
    if not skip_access:
        ...
        if result.returncode != 0:
            ...
            print(f"Remote SLURM/Pyxis/Enroot preflight failed: host={working_host}: {reason}")
            return False
...
def check_slurm_runtime(platform: dict[str, Any]) -> bool:
    ...
    if requested_time is None:
        ...
        print(f"SLURM runtime requested: partition={requested_partition}, SLURM_TIME_HOURS={requested_time:g}")
    ...
    if requested_timeout >= requested_time:
        print(
            f"SLURM_TIMEOUT_HOURS must be smaller than SLURM_TIME_HOURS ({requested_timeout:g} >= {requested_time:g})."
        )
        return False
    return True
...
def check_local_docker(
    ...
    if require_remote_docker and not remote_docker:
        print("Missing remote Docker requirement: set DOCKER_HOST to a remote daemon URL such as ssh://user@gpu-host.")
        return False
    if not skip_access:
        if not shutil.which("docker"):
            ...
            if runtime_query_ok and "nvidia" in runtimes:
                ...
                print("Docker NVIDIA runtime missing: install/configure NVIDIA Container Toolkit before launch.")
                ok = False
            else:
                ok = False
            ...
            if remote_docker:
                ...
                gpu_ok, gpus = query_docker_gpus(gpu_smoke_image, pull_smoke_image, target_gpu_indices)
            else:
            ...
            if gpu_ok:
                ...
                print("Docker GPU smoke skipped: effective GPU allocation failed validation")
...
def check_env_only(platform: dict[str, Any], paths: list[tuple[str, str]]) -> bool:
    ...
    for label, raw_path in paths:
        if raw_path.startswith(REMOTE_SCHEMES):
            ...
            print(f"Path provided for remote platform, verify it is mounted in the job: {label}={raw_path}")
    return True
