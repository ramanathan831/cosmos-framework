...
def _file_inventory(root: Path, names: Iterable[str], *, hash_content: bool = True) -> list[dict[str, Any]]:
    inventory = []
    for name in names:
        path = root / name
        if path.is_file():
...
def inspect_model(
    ...
    *,
        ...
        result.update(
            {
                ...
                "fingerprint_mode": (
                    "metadata_content_and_weight_sizes" if fast_weight_fingerprint else "full_content"
                ),
            }
        )
    ...
    if prepared:
        ...
        prepared_result = inspect_model(prepared, fast_weight_fingerprint=fast_weight_fingerprint)
...
def decode_media(path: Path) -> None:
    ...
    if not ffprobe:
        raise WorkflowError(f"media decode validation requires ffprobe on the inspection host; cannot validate {path}")
    ...
    *,
    ...
    for annotation_index, annotation_id in enumerate(annotation_ids):
        ...
        if dataset_family != "auto" and observed_family != dataset_family:
            schema_errors.append(f"{annotation_path}: detected {observed_family}, requested {dataset_family}")
    ...
    for task, values in answer_targets.items():
        ...
        if len(values) == task_counts[task] and any(normalized <= labels for labels in CLASSIFICATION_LABEL_SETS):
            task_metrics.setdefault(task, "accuracy")
            inferred_metrics[task] = "all conversation targets are deterministic classification labels"
    ...
    for task in sorted(task_counts):
        labels = {value.casefold().strip() for value in answer_targets.get(task, []) if value.strip()}
        ...
        task_semantics[task] = {
            ...
            "classification_vocabulary": (sorted(labels) if labels <= {"yes", "no", "a", "b", "c", "d"} else []),
            "declared_metric": task_metrics.get(task),
        }
    known_task_types = {value["task_type"] for value in task_semantics.values() if value["task_type"]}
    unresolved_accuracy_tasks = sorted(
        task for task, value in task_semantics.items() if task in accuracy_tasks and value["task_type"] is None
    )
    inferred_task_type = next(iter(known_task_types)) if len(known_task_types) == 1 else ""
    metric_names = sorted(
        {metric for metric in task_metrics.values() if metric not in {"accuracy", "exact_match_accuracy"}}
    )
    evaluation_profile = {
    ...
    profile = {
        ...
        "resolution": {
            ...
            "class": (
                ...
                if statistics.median(widths) * statistics.median(heights) <= 1280 * 720
                else "up_to_1080p"
                if statistics.median(widths) * statistics.median(heights) <= 1920 * 1080
                else "above_1080p"
            ),
        },
        "video": {
...
def model_parity(left: Mapping[str, Any], right: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "equivalent" if same else "invalid_mismatch",
...
def dataset_parity(left: Mapping[str, Any], right: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "equivalent" if same else "invalid_mismatch",
        "left": left.get("dataset_fingerprint"),
        "right": right.get("dataset_fingerprint"),
    }
...
def optimization_parity(left: Mapping[str, Any], right: Mapping[str, Any]) -> dict[str, Any]:
    ...
    differences = {
        key: {"left": left.get(key), "right": right.get(key)} for key in keys if left.get(key) != right.get(key)
    }
...
def validate_metadata(metadata: Mapping[str, Any]) -> None:
    ...
    if not isinstance(slurm, Mapping) or slurm_required - set(slurm):
        raise WorkflowError(
            f"SLURM metadata is incomplete; missing slurm fields: {sorted(slurm_required - set(slurm or {}))}"
        )
    if (
...
def _inspect_inputs_cli(argv: Sequence[str]) -> dict[str, Any]:
    ...
    runtime_paths = {label: planned_path_identity(path) for label, path in map(_split_labeled_path, args.runtime_path)}
    return {
        "schema_version": 1,
...
def materialize_dataset(
    *,
    ...
    normalized_tasks = {item.casefold().replace("-", "_").replace(" ", "_") for item in selected_tasks}
    ...
    for value in annotations:
        ...
        if dataset_family != "auto" and observed != dataset_family:
            raise WorkflowError(f"{annotation}: detected {observed}, requested {dataset_family}")
        for item in items:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        payload = (
            _inspect_inputs_cli(values[1:]) if values[0] == "inspect-inputs" else _materialize_dataset_cli(values[1:])
        )
