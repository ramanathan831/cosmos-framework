...
vision_process.VIDEO_READER_BACKENDS["torchvision"] = _read_video_torchvision_cached
...
def _prepare_chunk_inputs(evaluator, input_tasks):
    ...
    for task in input_tasks:
        ...
        if len(prompt) > 1 and prompt[1]["role"] == "user" and isinstance(prompt[1]["content"], str):
            content = []
        ...
        rendered = evaluator.processor.apply_chat_template(prompt, tokenize=False, add_generation_prompt=True)
        media_key = (
            media_mode,
            tuple(media_paths),
        ...
        if media_key not in media_payloads:
            ...
            if videos:
                ...
                media_payloads[media_key] = {"multi_modal_data": {"image": images}}
            elif media_paths:
                raise RuntimeError(f"Could not prepare media for evaluation task {task.get('id')}")
            else:
                media_payloads[media_key] = {}
...
def _run_evaluation_streaming(
    ...
    for current_shard_id in shard_ids:
        ...
        if skip_saved:
            pending = [
                (item, output) for item, output in zip(inputs, outputs) if not Path(output["output_path"]).exists()
            ]
        ...
        shard_losses = []
        total = len(inputs)
        logging.info("Streaming %d evaluation tasks in chunks of %d", total, chunk_size)
        for start in range(0, total, chunk_size):
            stop = min(start + chunk_size, total)
            chunk_inputs = inputs[start:stop]
            prepared = _prepare_chunk_inputs(self, chunk_inputs)
            predictions, losses = self.run_model_inference(prepared, chunk_inputs, answer_type)
            shard_predictions.extend(predictions)
            shard_losses.extend(losses)
            self._send_status_callback(f"Streaming evaluation: {stop}/{total} tasks completed")
    ...
    if all_losses:
        metrics.setdefault("overall", {})["loss"] = sum(all_losses) / len(all_losses)
    elapsed = time.time() - started
    logging.info("Streaming evaluation completed in %.2f seconds", elapsed)
    self._send_status_callback(f"Evaluation completed successfully in {elapsed:.1f} seconds")
    return metrics
