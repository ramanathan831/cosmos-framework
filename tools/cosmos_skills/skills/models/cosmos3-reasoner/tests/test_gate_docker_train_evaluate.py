...
def write_adapter(root: Path, config: dict) -> Path:
    ...
    (checkpoint / "adapter_model.safetensors").write_bytes(struct.pack("<Q", len(header)) + header + b"\0\0\0\0")
    return checkpoint
...
def write_dense(root: Path) -> Path:
    ...
    checkpoint.mkdir(parents=True)
    (checkpoint / "config.json").write_text(json.dumps({"model_type": "qwen3_vl"}), encoding="utf-8")
    header = json.dumps({"layer.weight": {"dtype": "F32", "shape": [1], "data_offsets": [0, 4]}}).encode()
    (checkpoint / "00000.safetensors").write_bytes(struct.pack("<Q", len(header)) + header + b"\0\0\0\0")
...
def test_validates_unique_adapter_checkpoint(tmp_path: Path) -> None:
    ...
    assert gate.find_and_validate_checkpoint(tmp_path / "results", 3, "adapter", expected_path) == checkpoint
...
def test_rejects_adapter_metadata_mismatch(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="adapter config mismatch"):
        gate.find_and_validate_checkpoint(tmp_path / "results", 3, "adapter", expected_path)


def test_validates_dense_checkpoint_and_index(tmp_path: Path) -> None:
    checkpoint = write_dense(tmp_path / "results")

    assert gate.find_and_validate_checkpoint(tmp_path / "results", 1, "dense") == checkpoint
