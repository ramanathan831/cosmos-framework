...
not package a `skills/models/cosmos3-reasoner/config.json` file.
...
```bash
bash skills/platform/cosmos-setup-gpu-host/scripts/setup-nvidia-gpu-host.sh \
...
skill = Path(os.environ["COSMOS_SKILLS_ROOT"]) / "skills/models/cosmos3-reasoner"
