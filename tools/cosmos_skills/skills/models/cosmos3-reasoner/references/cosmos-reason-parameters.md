...
For platform-side multi-node setup (sbatch flags on SLURM, Indexed Job + Service on Kubernetes), see the platform skill's "Multi-node training" section: `skills/platform/cosmos-run-on-slurm`, `skills/platform/cosmos-run-on-kubernetes`. Brev and local Docker are single-host only.
...
it as minimum-version overrides to `cosmos-setup-gpu-host`; do not copy the
