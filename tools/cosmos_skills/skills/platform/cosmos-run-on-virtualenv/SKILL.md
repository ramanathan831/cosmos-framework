...
- Single node only. For TAO container actions use `cosmos-run-on-docker`; for
...
on local disk (stage with `cosmos-data-io` first if they live in S3). Outputs land
in the job record's `results_dir`, which IS the runner's `--job-dir`.

## Execution — the four verbs

`$BANK` = `${COSMOS_SKILLS_ROOT}`; `$RUNNER` =
`$BANK/skills/platform/cosmos-run-on-virtualenv/references/virtualenv_runner.py`.
