...
`cosmos-run-on-slurm` is a platform **consumer**: it runs a spec-bundle over
...
`${COSMOS_SKILLS_ROOT}`; `$LOGIN` = a resolved `SLURM_HOSTNAME`.
...
1. **Reuse what's already staged — never redo (tier A):**
   ...
   - *Dataset:* **confirm it is already on Lustre** (`ssh $LOGIN test -e …`) and
     reference those paths; `cosmos-data-io` stages *only* a small auxiliary input
