...
if [ "${TAO_K8S_SKIP_NODE_RUNTIME_CHECK:-0}" != "1" ]; then
  COSMOS_SKILLS_ROOT="${COSMOS_SKILLS_ROOT:-$PWD}"
  SETUP_SCRIPT="${COSMOS_SKILLS_ROOT}/skills/platform/cosmos-setup-gpu-host/scripts/setup-nvidia-gpu-host.sh"
...
- **AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY / S3_BUCKET_NAME / S3_ENDPOINT_URL** (optional): for S3 dataset I/O (storage tier C), injected into the pod via the per-job Secret (`envFrom.secretRef`), never inline. Legacy `ACCESS_KEY`/`SECRET_KEY` are mapped by `cosmos-data-io`.
...
1. **GPU-capacity gate — hard-fail first** (no gang scheduling → a too-big Job
   ...
   ```
2. **Storage tier** (via `cosmos-data-io`): **A** = mount a bound PVC/NFS holding the
...
The `submit` verb refuses to launch GPU jobs on a cluster with no `nvidia.com/gpu` allocatable. For self-managed clusters, first run the `cosmos-setup-gpu-host` install action on every GPU worker node or bake the same package set into the node image:

```bash
bash skills/platform/cosmos-setup-gpu-host/scripts/setup-nvidia-gpu-host.sh --backend kubernetes --install --yes
```
