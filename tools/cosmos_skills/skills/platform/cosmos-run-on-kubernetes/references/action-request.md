...
if [ "$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["spec_bundle"]["mode"])' \
  ...
  CONFIG_SOURCE=$(python3 \
    "$BANK/skills/platform/cosmos-run-on-kubernetes/scripts/render_action_job.py" \
...
Use `cosmos-data-io` when the producer source is not already visible from the
...
python3 "$BANK/skills/platform/cosmos-run-on-kubernetes/scripts/render_action_job.py" \
