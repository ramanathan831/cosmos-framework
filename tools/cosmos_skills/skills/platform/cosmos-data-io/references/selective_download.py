...
def extract_keys(
    ...
    if missing:
        raise KeyError(f"annotation column(s) {missing} not found; available columns: {list(df.columns)}")
...
def download_selective(
    ...
    *,
    ...
    if errors:
        ...
        raise RuntimeError(f"{len(errors)} of {len(rel_keys)} objects failed to download: {preview}{more}")
    return results
