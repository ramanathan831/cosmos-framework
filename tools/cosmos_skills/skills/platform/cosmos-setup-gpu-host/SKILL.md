...
bash skills/platform/cosmos-setup-gpu-host/scripts/setup-nvidia-gpu-host.sh --backend docker --check-only

# Install or repair after user approval.
bash skills/platform/cosmos-setup-gpu-host/scripts/setup-nvidia-gpu-host.sh --backend docker --install

# Check a Kubernetes GPU worker host.
bash skills/platform/cosmos-setup-gpu-host/scripts/setup-nvidia-gpu-host.sh --backend kubernetes --check-only
```
