...
contract**, deferring the container-how to `cosmos-run-on-docker` over `brev exec`.
...
No shared NFS/Lustre — storage tier **B/C** via `cosmos-data-io`: stage inputs from
...
container-how to the four docker verbs** (`cosmos-run-on-docker`) run over
`brev exec`. It is not a symmetric peer — teardown must additionally delete the
instance to stop billing. `$BANK` = `${COSMOS_SKILLS_ROOT}`.
