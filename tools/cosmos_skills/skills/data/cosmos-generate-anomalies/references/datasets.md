...
Obtain the dataset zip from the
[Roboflow dataset page](https://universe.roboflow.com/vu-thi-thu-huyen/mobile-screen)
using the user's authorized access and the site's export instructions. The
original workflow's separate download PDF was not included in the source
bundle; it is not a prerequisite for the preparation command below.
