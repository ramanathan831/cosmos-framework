---
name: cosmos-mine-aoi-images
description: Embed inspection gap targets and a source image pool, then mine nearest neighbors with history-aware deduplication for Cosmos DEFT AOI iterations. Use when expanding a real-image training queue from the selected mining pool.
license: Apache-2.0
metadata:
...
1. **Target parquet** — the Cosmos DEFT AOI routing output, derived from Proxy gaps and joined back to the source annotation by id. Required column: `filepath`. If `label` is also present, label-aware filtering during mining is available; otherwise the mining task silently no-ops the filter.
...
See `references/setup.md` for the full environment notes, `COSMOS_SKILLS_ROOT` handling, the path-mounting rationale, the `getpwuid` chown workaround, the CSV-to-parquet snippet, and the verbatim spec-file authoring blocks.
...
```bash
python3 skills/data/cosmos-mine-aoi-images/scripts/filter_mined_history.py \
