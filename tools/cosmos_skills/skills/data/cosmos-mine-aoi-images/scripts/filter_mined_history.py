...
def _validated_history(path: pathlib.Path, iteration: int) -> dict[str, Any]:
    if not path.is_file():
        if iteration != 1:
            raise FileNotFoundError(f"mining history is required before iteration {iteration}: {path}")
        return {
    ...
    for entry in entries:
        ...
        if not isinstance(selected, list):
            raise ValueError(f"mining history iteration {number} has no selected_filepaths list")
        normalized = [_normalize_filepath(value) for value in selected]
        if len(normalized) != len(set(normalized)):
            raise ValueError(f"mining history iteration {number} contains duplicate filepaths")
        overlap = all_selected.intersection(normalized)
        if overlap:
            raise ValueError(f"mining history iteration {number} reselects prior filepaths: {sorted(overlap)[:3]}")
        if int(entry.get("selected_count", -1)) != len(normalized):
            raise ValueError(f"mining history iteration {number} selected_count disagrees with selected_filepaths")
        for path_field, hash_field in (
            ...
            if not artifact.is_absolute() or not artifact.is_file() or not expected_hash:
                raise ValueError(f"mining history iteration {number} has invalid {path_field} proof")
            actual_hash = _sha256(artifact)
            if actual_hash != expected_hash:
                raise ValueError(f"mining history iteration {number} {path_field} hash mismatch")
        entry["selected_filepaths"] = normalized
        all_selected.update(normalized)

    if actual_numbers != expected_numbers:
        raise ValueError(f"mining history iterations must be contiguous from 1; found {actual_numbers}")
    if payload.get("cumulative_unique_count") != len(all_selected):
        raise ValueError("mining history cumulative_unique_count disagrees with selected_filepaths")
    return payload
...
def select_novel_samples(
    *,
    ...
    committed = next((entry for entry in entries if int(entry["iteration"]) == iteration), None)
    if committed is not None:
        if not resume:
            raise ValueError(
                f"iteration {iteration} is already committed in {history_file}; pass --resume to verify and reuse it"
            )
        ...
        if pathlib.Path(committed["candidate_parquet"]).resolve() != candidate_parquet:
            raise ValueError("committed candidate parquet does not match --candidate-parquet")
    ...
    if len(entries) != iteration - 1:
        raise ValueError(f"cannot select iteration {iteration}; mining history ends at iteration {len(entries)}")

    table = pq.read_table(candidate_parquet)
    if filepath_column not in table.column_names:
        raise ValueError(f"candidate parquet is missing {filepath_column!r}; columns={table.column_names}")

    historical = {value for entry in entries for value in entry["selected_filepaths"]}
    ...
    historical_candidate_rate = already_mined_count / len(candidate_seen) if candidate_seen else 0.0
    recommendation = None
    if not selected_names and candidate_seen:
        recommendation = "all candidates were selected in earlier iterations; increase topn or expand the source pool"
    elif historical_candidate_rate >= 0.5:
        recommendation = (
            "at least half of the unique candidates were already mined; consider increasing topn to cast a wider net"
        )
