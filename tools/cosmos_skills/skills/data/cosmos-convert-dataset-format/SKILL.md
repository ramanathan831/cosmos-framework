---
name: cosmos-convert-dataset-format
description: Convert DAFT datasets into supported Cosmos reasoning training formats with `tao-daft convert`. Use for DAFT-to-Cosmos dataset preparation or an explicit DAFT conversion request. Not for native framework JSONL assembly or arbitrary non-DAFT inputs.
license: Apache-2.0
metadata:
...
# Convert DAFT Datasets for Cosmos

> **Bundle setup:** Use `cosmos-workflow-setup` to resolve this checkout's `tools/cosmos_skills` root. No deprecated Skill Bank installation is required.

For native framework JSONL assembly, use the repository's existing
`cosmos3-post-training` skill and `docs/dataset_jsonl.md` instead. Inspect the
conversion command first, then obtain authorization before writing the output.
...
4. Infer source from layout (same directory markers as the
   `cosmos-validate-dataset-format` skill's "Format inference"). If you cannot infer
...
the `cosmos-validate-dataset-format` skill before training. For large trees, capture
