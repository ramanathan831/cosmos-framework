---
name: cosmos-validate-dataset-format
description: Validate DAFT datasets used for Cosmos reasoning, including supported cosmos-reason formats, with `tao-daft validate`. Use for DAFT structure, schema, and media cross-reference checks. Not for arbitrary non-DAFT or native framework JSONL validation.
license: Apache-2.0
metadata:
...
# Validate DAFT Datasets for Cosmos
...
with `tao-daft info`, consult that installed CLI's `--help`; no separate bundled
inspection skill is required.
...
- Covers `validate` only. Use `cosmos-convert-dataset-format` for conversion
  and the installed `tao-daft info --help` for inspection.
