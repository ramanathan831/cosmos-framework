...
The `cosmos-launch-workflow` skill points here for the full per-platform preflight steps.
...
```bash
${COSMOS_SKILLS_ROOT:?}/scripts/check_tao_launch_preflight.py \
...
6. For multi-GPU Slurm jobs, request `--gpus-per-node=<N>` (the
   `cosmos-run-on-slurm` platform skill emits this in the sbatch launcher). Do not
