...
metadata:
  ...
  version: 0.1.1
  compatibility: Requires the packaged Cosmos workflow bundle helper scripts.
  tags:
  - tao
  - workflow
  - launch
...
Use this skill before launching a model, data, or application action from this
bundle. Native framework development and read-only planning do not become
managed launches merely because the repository contains these skills.
...
spec-bundle (schema: `cosmos-artifacts`), execution is exactly four verbs. Every
platform skill implements them over its native CLI — the bank ships five
(`cosmos-run-on-docker`, `-slurm`, `-kubernetes`, `-brev`, `-virtualenv`), and any
...
- **submit(spec-bundle)** — resolve the data question first: if the inputs are
  **already readable from the compute frame** (a local path, an existing mount —
  ...
  nothing to stage — record tier A and move on. Invoke `cosmos-data-io` only on a
...
session** — you already see them by name and description (`cosmos-run-on-docker`,
...
platform skills by reading `skills/platform/cosmos-run-on-*/SKILL.md` frontmatter
(name + one-line description) under `${COSMOS_SKILLS_ROOT}`.
...
```bash
${COSMOS_SKILLS_ROOT:?}/scripts/resolve_tao_image.py \
