...
This is a skill-bank **workflow** skill at `skills/applications/cosmos-automl/`. The agent
...
| SDK | Best for AutoML |
|---|---|
| `BrevSDK` | Cost-tuned sweeps on Brev instances (single-instance per rec, multi-GPU OK). Multi-credential / multi-workspace accounts must pass `cloud_cred_id=` and `workspace_group_id=` to `create_job` — see `skills/platform/cosmos-run-on-brev/SKILL.md`. |
...
(For local development against a checkout: `pip install -e '~/cosmos-automl[brev]'` from the cloned repo.)
...
3. **Workflow bundle available**: the runner takes an explicit `skill_dir` — the **absolute path to a model directory** in this checkout, such as `$COSMOS_SKILLS_ROOT/skills/models/cosmos3-reasoner`. Pass it per run; the runner does not resolve environment variables itself. Source `tools/cosmos_skills/env.sh` to locate the bundle, then resolve user model aliases before constructing the path. Do not infer the skill-directory name from `network_arch` or install the deprecated Skill Bank plugin.
   ```python
...
4. **`nvidia-tao-automl` installed** with the platform extra you want. Resolve
   ...
   ```
   For local development against a checkout: `pip install -e '~/cosmos-automl[brev]'`.
...
```bash
${COSMOS_SKILLS_ROOT:?}/scripts/list_tao_models.py \
  --skill-bank ${COSMOS_SKILLS_ROOT:?} --scope automl --format text
```

The compatibility wrapper below is also valid and delegates to the same logic:

```bash
${COSMOS_SKILLS_ROOT:?}/scripts/list_automl_support.py \
  --skill-bank ${COSMOS_SKILLS_ROOT:?} --format text
```
