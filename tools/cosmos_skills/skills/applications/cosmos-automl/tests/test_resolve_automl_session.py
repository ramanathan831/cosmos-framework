...
def test_explicit_identity_resumes_real_automl_controller(tmp_path):
    ...
    schema_path = Path(__file__).resolve().parents[3] / "models/cosmos3-reasoner/schemas/train.schema.json"
    ...
    assert [path.name for path in (workspace / ".automl/controller").glob("*.json")] == [f"{session_id}.json"]
