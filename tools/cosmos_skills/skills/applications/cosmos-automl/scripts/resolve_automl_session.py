...
def _controller_files(workspace: Path) -> list[Path]:
    ...
    if not controller_dir.is_dir():
        raise SessionResolutionError(
            f"resume state is missing: {controller_dir} does not exist; refuse to start a fresh search with resume=True"
        )
...
def _validate_controller(path: Path) -> None:
    try:
        ...
        raise SessionResolutionError(f"controller state is unreadable: {path}: {exc}") from exc
    if not isinstance(state, list):
        raise SessionResolutionError(f"controller state has unexpected shape: {path} must contain a JSON list")
...
def resolve_session_id(workspace: Path, requested: str | None = None) -> str:
    ...
    if requested is not None:
        ...
        if selected not in files:
            ...
            raise SessionResolutionError(
                f"session {requested!r} has no controller state in {workspace}; available sessions: {available}"
            )
    else:
        if not files:
            raise SessionResolutionError(
    ...
    if not SESSION_ID_RE.fullmatch(selected.stem):
        raise SessionResolutionError(f"controller filename is not a valid session id: {selected.name}")
...
def validate_session_settings(
    ...
    *,
    ...
    if not isinstance(session_id, str) or not SESSION_ID_RE.fullmatch(session_id):
        raise SessionResolutionError("automl_settings.session_id must be an explicit valid session id")
    if resume:
        if workspace is None:
            raise SessionResolutionError("resume=True requires the full existing workspace path")
        resolved = resolve_session_id(workspace, session_id)
        if resolved != session_id:  # defensive: requested resolution must be exact
            raise SessionResolutionError(f"resolved session {resolved!r} does not match settings {session_id!r}")
    return session_id
...
def build_parser() -> argparse.ArgumentParser:
    ...
    resume = subparsers.add_parser("resolve", help="recover the session id from an existing run workspace")
