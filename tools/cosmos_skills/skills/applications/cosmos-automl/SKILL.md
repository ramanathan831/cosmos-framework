---
name: cosmos-automl
description: Run explicit Cosmos hyperparameter or prompt optimization with AutoMLRunner, packaged model schemas, evaluation hooks, and the selected execution platform. Use for Cosmos HPO, AutoML, or prompt/config search. Preserve a matching Cosmos DEFT workflow for iterative data improvement; Cosmos Embed has no AutoML schema.
license: Apache-2.0
metadata:
...
> Use `cosmos-workflow-setup` to locate this checkout's bundle and runtime prerequisites.
...
- `skills/core/cosmos-launch-workflow/SKILL.md`: shared intake pattern for platform,
...
skill_bank = Path("<absolute-cosmos-workflow-bundle>")
model_skill = "<resolved-model-skill-directory>"
skill_dir = skill_bank / "skills" / "models" / model_skill
sys.path.insert(0, str(skill_bank / "skills/applications/cosmos-automl/scripts"))
