...
3. Read `execution/platforms/<platform>/guide.md` and start the container (Section 4.2).
...
**To stop a service:** Read `references/code-templates.yaml` → `stop.registry_read` to resolve the job_id, read `execution/platforms/<platform>/guide.md`, then follow Section 5.
...
## Secrets rule (applies to every generated code block in this reference)
...
**`CLOUD_BASED`** — always `"False"` for this reference (disables callback posting to `TAO_LOGGING_SERVER_URL`).
...
The job payload and inner command (Sections 1–3) are **platform-agnostic**. For each platform, read **`execution/platforms/<name>/guide.md`** for preflight checks and credentials **before** generating any execution code.
...
Read **`execution/platforms/<platform>/guide.md`** and follow it to start the container.
...
| **brev** | `instance_id` (optional — reuse an existing instance); on multi-credential / multi-workspace accounts also `cloud_cred_id` and `workspace_group_id` for first-create — see `execution/platforms/brev/guide.md` |
...
Ask the user for the `job_id` to stop. If they don't provide one, default to `state["latest"]` and confirm which job_id is being stopped. Read the registry using `references/code-templates.yaml` → `stop.registry_read`, then read **`execution/platforms/<platform>/guide.md`** and use its cancellation / stop mechanism.
