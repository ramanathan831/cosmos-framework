...
Load this only when `guide.md` points here. If this conflicts with `guide.md`, `skill_info.yaml`, schemas, or platform/model skills, the current/compact source wins.
...
For platform-side multi-node setup (sbatch flags on SLURM, Indexed Job + Service on Kubernetes), see the platform skill's "Multi-node training" section: `execution/platforms/slurm`, `execution/platforms/kubernetes`. Brev and local Docker are single-host only.
...
it as minimum-version overrides to `execution/gpu-host/guide.md`; do not copy the
