...
# Reasoner training and evaluation reference

This reference extends `cosmos3-post-training`; it is not a separate skill.
Framework requests select `--backend cosmos-framework` explicitly. The backend
table below documents the imported helper's compatibility behavior, not a reason
to change the user's requested implementation. Native generator recipes remain
in the existing post-training skill.

Read `references/cosmos-data-specs.md` for conversation/task-aware formats;
`references/cosmos-reason-evaluate.md` for evaluation; and
`references/cosmos-backend-operations.md` for checkpoint/runtime operations.
...
that the selected backend will prepare its compatible checkpoint automatically, name the
