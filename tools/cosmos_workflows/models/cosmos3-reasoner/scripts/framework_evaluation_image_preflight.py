...
def _read_installed_sources(sqsh: Path, listing: str) -> dict[str, str]:
    ...
    for suffix, listed_path in _installed_paths(listing).items():
        ...
        if completed.returncode:
            raise RuntimeError(f"unable to read baked evaluator module {suffix}: {completed.stderr.strip()}")
...
def main() -> int:
    ...
    try:
        if args.listing_file:
            ...
            if completed.returncode:
                raise RuntimeError(f"unable to inspect SQSH file list: {completed.stderr.strip()}")
