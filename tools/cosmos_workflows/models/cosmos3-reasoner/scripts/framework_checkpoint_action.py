...
def infer_config_file(checkpoint: Path) -> Path:
    ...
    raise WorkflowError(
        f"Framework DCP checkpoint configuration could not be inferred; supply --config-file for {checkpoint}"
    )
...
def _safetensors_file_tensor_keys(path: Path) -> set[str]:
    ...
    try:
        ...
        with path.open("rb") as stream:
            ...
            if header_length <= 0 or header_length > min(100 * 1024 * 1024, size - 8):
                raise WorkflowError(f"safetensors file has an invalid header length: {path}")
    ...
    tensor_entries = {name: value for name, value in header.items() if name != "__metadata__"}
    if not tensor_entries or not all(
        isinstance(name, str) and name and isinstance(value, dict) for name, value in tensor_entries.items()
    ):
...
def _safetensors_tensor_keys(root: Path) -> set[str] | None:
    ...
    for weight_file in weight_files:
        try:
            ...
            raise WorkflowError(f"safetensors weight path escapes or is missing below {root}: {weight_file}") from exc
        keys = _safetensors_file_tensor_keys(resolved_weight)
        duplicates = actual_keys & keys
        if duplicates:
            raise WorkflowError(f"duplicate tensor keys across safetensors files: {sorted(duplicates)[:10]}")
...
def _default_export_dir(checkpoint: Path, config_file: Path, metadata: Path) -> Path:
    fingerprint = hashlib.sha256((sha256_file(metadata) + sha256_file(config_file)).encode("ascii")).hexdigest()[:16]
...
def build_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    config = (
        Path(args.config_file).expanduser().resolve(strict=True) if args.config_file else infer_config_file(checkpoint)
    )
    export = (
    ...
    if args.base_model_path_or_uri:
        export_command.extend(["--base-model-path-or-uri", _normalized_source(args.base_model_path_or_uri)])
