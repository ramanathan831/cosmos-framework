...
def _visual_gradient_event(record: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    for component in VLM_COMPONENTS:
        ...
        component_values = {
            field: values[f"{prefix}/{field}"] for field in COMPONENT_FIELDS if f"{prefix}/{field}" in values
        }
        if component_values:
...
def load_evaluation(path: Path | None) -> dict[str, Any]:
    ...
    if accuracy is None or correct is None or total is None or total <= 0:
        raise MetricError(
            "evaluation output must contain finite overall accuracy, correct numerator, and positive total denominator"
        )
    ...
    return {
        ...
        "excluded_tasks": value.get("excluded_tasks", []),
...
def summarize_records(
    ...
    for record in records:
        ...
        if phase in {"checkpoint_saved", "checkpoint_complete", "checkpoint_submitted"} or checkpoint_path:
            checkpoints.append(
                {"path": checkpoint_path, "phase": phase, "epoch": values.get("epoch", record.get("epoch"))}
            )
        # Step/heartbeat train loss is intentionally never treated as complete-run average.
        if "train/loss_numerator" in values and "train/valid_label_count" in values:
    ...
    terminal = next(
        (state for state in reversed(states) if state in SUCCESS | FAILURE), states[-1] if states else "UNKNOWN"
    )
    ...
    return {
        ...
        "first_update_visual_gradient_contract": (visual_gradient_events[0] if visual_gradient_events else None),
        "evaluation": evaluation_summary,
        "checkpoints": checkpoints,
        "failures": failures,
        "missing": missing,
        "source": "structured_status_and_repository_evaluator",
    }
...
def diagnostic_log(path: Path) -> dict[str, Any]:
    ...
    signatures = {
        ...
        "nccl": "nccl",
        "decoder": "nvcuvid",
        "python_permission": "permission denied",
        "traceback": "traceback",
    }
    return {
        "source": "log_diagnostic_only",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.format == "text":
        ...
        print(f"first-update visual-gradient contract: {summary['first_update_visual_gradient_contract']}")
    else:
