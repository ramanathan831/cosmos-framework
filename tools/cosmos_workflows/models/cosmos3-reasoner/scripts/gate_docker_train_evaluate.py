...
def find_and_validate_checkpoint(
    ...
    if checkpoint_kind == "dense":
        ...
        validate_safetensors(weights_path)
    LOG.info("validated epoch-%d %s checkpoint at %s", epoch, checkpoint_kind, checkpoint)
    return checkpoint
    ...
    with status_path.open(encoding="utf-8") as stream:
        for line_number, line in enumerate(stream, start=1):
            ...
            try:
                ...
                raise ValueError(f"invalid structured status JSON at {status_path}:{line_number}: {exc}") from exc
            status = event.get("status") if isinstance(event, dict) else None
            if status in {"SUCCESS", "FAILURE"}:
                terminal = status
    if terminal != "SUCCESS":
        raise ValueError(f"training structured terminal status is {terminal or 'missing'}, expected SUCCESS")
...
def launch_eval(args: argparse.Namespace, job_id: str, results_dir: Path, checkpoint: Path) -> str:
    ...
    lint = run([sys.executable, str(args.redactor), "lint", str(args.eval_spec)], check=False)
    if lint.returncode:
...
def find_results(results_dir: Path) -> dict[str, Any]:
    ...
    if isinstance(payload, dict):
        ...
        correct = sum(str(item["response"]).strip() == str(item["gt"]).strip() for item in payload)
        result = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint-kind", choices=("adapter", "dense"), default="adapter")
...
def main() -> int:
    ...
    try:
        ...
        if not eval_container:
            ...
            if not eval_container:
                eval_container = launch_eval(args, eval_job, eval_results_dir, Path(state["checkpoint"]))
            else:
                mark(
                    args.job_helper,
                    eval_job,
