...
def resolve_model_profile(
    ...
    if args.min_frames is not None and args.max_frames is not None and args.min_frames > args.max_frames:
        raise WorkflowError("min_frames must not exceed max_frames")
    if (args.video_start is not None and args.video_start < 0) or (args.video_end is not None and args.video_end < 0):
        raise WorkflowError("video_start and video_end must be nonnegative")
    if args.video_start is not None and args.video_end is not None and args.video_start >= args.video_end:
    ...
    if args.video_min_pixels is not None and args.video_max_pixels and args.video_min_pixels > args.video_max_pixels:
...
def _remote_sqsh_missing_entries(
    ...
    *,
    ...
    if MODEL_PREPARATION_RUNTIME_ENTRY in known_entries and MODEL_PREPARATION_RUNTIME_ENTRY not in missing_entries:
        ...
        validation_script = f"set -o pipefail; ( {reader_script} ) | python3 -c " + shlex.quote(
            "import json,sys; "
        ...
        if validation.returncode or validation.stdout.strip() != MODEL_PREPARATION_RUNTIME_VALIDATION_MODE:
...
def _rl_video_runtime(
    ...
    if requested == "auto":
        selected = "pynv-device-rgbp" if args.dataset_family == "video_conversation" else "system-pyav"
        reason = (
            "video_conversation defaults to the source-baked device-RGBP throughput profile"
    ...
    if workers and prefetch <= 0:
        raise WorkflowError("rl_dataloader_prefetch_factor must be positive when workers are enabled")
    ...
    if validation_shard_requested not in {"auto", "stride", "media_grouped"}:
        raise WorkflowError("rl_validation_shard_strategy must be auto, stride, or media_grouped")
    validation_shard_strategy = (
        "media_grouped"
    ...
    validation_feature_cache_override = getattr(args, "rl_validation_video_feature_cache_size", None)
    if validation_feature_cache_override is not None and validation_feature_cache_override < 0:
        raise WorkflowError("Cosmos-RL validation video feature cache size must be nonnegative")
    ...
    if batch_threads_override < 0:
        raise WorkflowError("Cosmos-RL video cache sizes and SFT batch threads must be nonnegative")
    if not fast and (video_cache_override is not None or decoder_cache_override is not None):
        raise WorkflowError("Cosmos-RL PyNv cache overrides require --rl-video-profile pynv-device-rgbp")
    ...
    video_cache_size = (
        unique_media_capacity if fast and video_cache_override is None else (video_cache_override if fast else 0)
    )
    decoder_cache_size = (
        unique_media_capacity if fast and decoder_cache_override is None else (decoder_cache_override if fast else 1)
    )
...
def _rl_spec(
    ...
    use_dataset_cache = args.dataset_family == "video_conversation" and cache_mode == "prewarm"
    ...
    if train_batch_per_replica % args.rl_mini_batch:
        raise WorkflowError("rl_train_batch_per_replica must be divisible by rl_mini_batch")
    ...
    spec["train"].update(
        {
            ...
            "optm_warmup_epochs": (int(args.warmup) if float(args.warmup).is_integer() else args.warmup),
    ...
    if validation_shard_strategy not in {"stride", "media_grouped"}:
        raise WorkflowError("rl_validation_shard_strategy must be stride or media_grouped")
    cache_frontload_batch_size = int(getattr(args, "rl_validation_cache_frontload_batch_size", 0))
    cache_frontload_unique_per_batch = int(getattr(args, "rl_validation_cache_frontload_unique_per_batch", 0))
    if bool(cache_frontload_batch_size) != bool(cache_frontload_unique_per_batch):
        raise WorkflowError("staged validation cache frontloading requires both batch size and unique-per-batch")
    if cache_frontload_batch_size:
        if validation_shard_strategy != "media_grouped":
            raise WorkflowError("staged validation cache frontloading requires media_grouped sharding")
        if cache_frontload_batch_size != args.validation_batch_size:
            raise WorkflowError("validation cache frontload batch size must equal validation_batch_size")
        if cache_frontload_unique_per_batch > cache_frontload_batch_size:
            raise WorkflowError("validation cache frontload unique-per-batch cannot exceed batch size")
    ...
    if dataloader_num_workers:
        spec["train"]["train_policy"]["dataloader_prefetch_factor"] = dataloader_prefetch_factor
    else:
    ...
    if prewarmed_cache:
        ...
        if missing_cache_keys:
            raise WorkflowError(f"full Cosmos-RL cache keys are missing: {sorted(missing_cache_keys)}")
        spec["train"]["train_policy"].update(
            {
    ...
    spec["custom"].update(
        {
            "train_dataset": {
                ...
                "response_mode": "hybrid" if args.dataset_family == "task_aware_video_reasoning" else "answer",
            },
            "val_dataset": {
            ...
            "validation_cache_frontload_unique_per_batch": (cache_frontload_unique_per_batch),
            "system_prompt": args.system_prompt,
        }
    )
...
def _env(
    ...
    if backend == "cosmos-framework":
        ...
        framework_train_media = (
            list(train_media) * len(train_annotations) if len(train_media) == 1 else list(train_media)
        )
    ...
    *,
    ...
    if force_all_validation_media:
        for annotation in args.validation_annotation:
            preparation_arguments.extend(["--force-annotation", _containerize(args, annotation)])
    ...
    if force_all_validation_media:
        for annotation in args.validation_annotation:
            validation_arguments.extend(["--require-covered-annotation", _containerize(args, annotation)])
...
def build_plan(
    ...
    *,
    ...
    if backend == "cosmos-framework" and getattr(args, "rl_baked_overlay_pythonpath", ""):
        raise WorkflowError("--rl-baked-overlay-pythonpath applies only to the cosmos-rl backend")
    if backend == "cosmos-rl" and getattr(args, "framework_baked_overlay_pythonpath", ""):
        raise WorkflowError("--framework-baked-overlay-pythonpath applies only to the cosmos-framework backend")
    ...
    if backend == "cosmos-framework" and any(
        ...
        raise WorkflowError("Cosmos-RL validation cache frontloading does not apply to cosmos-framework")
    if backend == "cosmos-framework" and any(
    ...
    if backend == "cosmos-rl" and getattr(args, "framework_validation_shard_strategy", "auto") != "auto":
        raise WorkflowError("Framework validation sharding applies only to cosmos-framework")
    ...
    rl_video_runtime = _rl_video_runtime(args, train_data, val_data) if backend == "cosmos-rl" else None
    ...
    framework_video_runtime = (
        _framework_video_runtime(args, train_data, val_data, framework_runtime_model_type)
        if backend == "cosmos-framework"
        else None
    )
    ...
    cache_prewarm_required = (
        backend == "cosmos-rl" and args.dataset_family == "video_conversation" and cache_mode == "prewarm"
    )
    ...
    *,
    ...
    if plan["backend"] == "cosmos-rl":
        for split, marker, key in (
            ...
            if args.run_mode == "smoke":
                smoke_limit = args.smoke_train_samples if split == "train" else args.smoke_validation_samples
            elif args.run_mode == "diagnostic":
                smoke_limit = args.train_sample_limit if split == "train" else args.validation_sample_limit
            else:
                smoke_limit = 0
...
def parity_report(left: Mapping[str, Any], right: Mapping[str, Any]) -> dict[str, Any]:
    ...
    left_decoder = {key: left.get("decoder_artifact", {}).get(key) for key in decoder_keys}
    right_decoder = {key: right.get("decoder_artifact", {}).get(key) for key in decoder_keys}
    if left_decoder["enabled"] and right_decoder["enabled"]:
        ...
        decoder_equal = left_decoder["input_fingerprints"] == right_decoder["input_fingerprints"] and left_decoder[
            "policy"
        ].get("forced_runtime_sources", []) == right_decoder["policy"].get("forced_runtime_sources", [])
    else:
        decoder_equal = False
