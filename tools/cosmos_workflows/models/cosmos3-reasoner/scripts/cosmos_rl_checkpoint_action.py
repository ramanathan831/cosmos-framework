...
def resolve_action_checkpoint(source: Path, epoch: int | None) -> tuple[Path, int]:
    ...
    if epoch is not None and inferred is not None and epoch != inferred:
        raise CheckpointError(f"requested epoch {epoch} conflicts with source checkpoint epoch {inferred}: {source}")
    selected_epoch = epoch if epoch is not None else inferred
    if selected_epoch is None:
        raise CheckpointError(f"cannot infer epoch from source checkpoint; supply --epoch: {source}")
    ...
    parts = source.parts
    try:
        checkpoint_index = max(index for index, value in enumerate(parts) if value == "checkpoints")
    except ValueError as exc:
        raise CheckpointError(
            f"native Cosmos-RL checkpoint must contain a checkpoints/epoch_N segment: {source}"
        ) from exc
    if checkpoint_index + 1 >= len(parts) or parts[checkpoint_index + 1] != f"epoch_{selected_epoch}":
        raise CheckpointError(f"source checkpoint does not identify epoch_{selected_epoch}: {source}")
...
def _validate_safetensors(path: Path) -> None:
    ...
    with path.open("rb") as stream:
        ...
        if header_size <= 2 or header_size > size - 8:
            raise CheckpointError(f"invalid safetensors header length in {path}: {header_size}")
        try:
...
def _validate_dense(checkpoint: Path) -> list[Path]:
    ...
    if index_path.is_file():
        ...
        if not all(
            isinstance(name, str) and not Path(name).is_absolute() and ".." not in Path(name).parts for name in names
        ):
...
def verify(
    ...
    *,
    ...
    if not action_checkpoint.is_dir():
        raise CheckpointError(f"evaluator-loadable epoch export is missing: {action_checkpoint}")
    if training_mode == "dense":
        files = _validate_dense(action_checkpoint)
        checkpoint_kind = "hf_dense_base_model_safetensors" if base_model else "hf_dense_safetensors"
