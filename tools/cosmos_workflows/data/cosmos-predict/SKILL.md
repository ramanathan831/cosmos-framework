...
| Generation settings | No | If the user provides a generation settings JSON, use it. Otherwise set `GENERATION_SETTINGS` to `data/cosmos-predict/assets/default_generation_settings.json`. Always pass the resolved path to `prepare_paidf_config.py` with `--generation-settings`. |
...
- `GENERATION_SETTINGS`: user-provided generation settings JSON, or `data/cosmos-predict/assets/default_generation_settings.json` when the user did not provide one.
...
GENERATION_SETTINGS="data/cosmos-predict/assets/default_generation_settings.json"
...
```bash
python data/cosmos-predict/scripts/verify_vlm_captioning_base_url.py \
...
```bash
python data/cosmos-predict/scripts/prepare_paidf_config.py \
...
```bash
python data/cosmos-predict/scripts/write_paidf_handoff.py \
  --input-jsonl "$INPUT_JSONL" \
