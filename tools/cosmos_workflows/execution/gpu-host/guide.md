...
bash execution/gpu-host/scripts/setup-nvidia-gpu-host.sh --backend docker --check-only

# Install or repair after user approval.
bash execution/gpu-host/scripts/setup-nvidia-gpu-host.sh --backend docker --install

# Check a Kubernetes GPU worker host.
bash execution/gpu-host/scripts/setup-nvidia-gpu-host.sh --backend kubernetes --check-only
```
