...
def _absolute_clean_path(value: Any, label: str) -> str:
    ...
    if not path.startswith("/") or path == "/" or "\\" in path or any(ord(ch) < 32 for ch in path):
...
def _config_content(bundle: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if raw_format not in CONFIG_FORMATS:
        raise RenderError("spec_bundle.config_format must be one of json, toml, or yaml")
    ...
    try:
        if raw_format == "toml":
            ...
            content = json.dumps(spec, allow_nan=False, indent=2, sort_keys=True) + "\n"
...
def materialize_config(request: dict[str, Any], output_dir: pathlib.Path) -> pathlib.Path:
    """Write one content-addressed config file for later PVC staging."""
    if request.get("schema_version") != REQUEST_SCHEMA_VERSION:
        raise RenderError(f"request.schema_version must be {REQUEST_SCHEMA_VERSION!r}")
...
def _command_bundle(request: dict[str, Any], config_path: str | None) -> tuple[list[str], list[str], str, int]:
    bundle = request.get("spec_bundle")
    if not isinstance(bundle, dict):
        raise RenderError("request.spec_bundle must be an object")
    ...
    if mode == "args":
        if "spec" in bundle or "config_format" in bundle:
            raise RenderError("spec_bundle.mode=args must not carry spec or config_format")
        ...
        args = [_string_value(value, f"spec_bundle.args[{index}]") for index, value in enumerate(raw_args)]
    elif mode == "config":
        if "args" in bundle:
            raise RenderError("spec_bundle.mode=config must not carry args")
        ...
        if "{config_path}" not in command_text:
            raise RenderError("spec_bundle.mode=config requires {config_path} in command")
...
def _staged_sources(payload: dict[str, Any]) -> dict[str, str]:
    ...
    for index, row in enumerate(rows):
        ...
        if sub_path in sub_paths:
            raise RenderError(
                f"staging map assigns one PVC subPath to distinct sources: {sub_paths[sub_path]} and {source}"
            )
...
def _mounts(
    ...
    rendered: list[dict[str, Any]] = [{"name": "dshm", "mountPath": "/dev/shm", "readOnly": False}]
    ...
    if config_mount is not None:
        ...
        if source in used_sources:
            raise RenderError("materialized config must have its own staged file source")
        if source not in staged:
            raise RenderError(f"materialized config has no staged PVC subPath: {source}")
        if target in targets:
            raise RenderError(f"materialized config repeats Kubernetes mount target: {target}")
        used_sources.add(source)
        targets.add(target)
        rendered.append(
            {
...
def _require_writable_outputs(request: dict[str, Any], source_modes: list[tuple[pathlib.PurePosixPath, bool]]) -> None:
    outputs = request.get("fresh_outputs", [])
    if not isinstance(outputs, list):
        raise RenderError("request.fresh_outputs must be an array")
    for index, raw in enumerate(outputs):
        output = pathlib.PurePosixPath(_absolute_clean_path(raw, f"fresh_outputs[{index}]"))
        writable = False
        for source, read_only in source_modes:
            ...
            if not read_only:
                ...
                break
        if not writable:
            raise RenderError(f"fresh output is not covered by a writable declared mount: {output}")


def _environment(request: dict[str, Any], credential_secret: str | None) -> list[dict[str, Any]]:
    ...
    if forward and credential_secret is None:
        raise RenderError("request.forward_env is non-empty but no --credential-secret was supplied")
    if not forward and credential_secret is not None:
        raise RenderError("--credential-secret must be omitted when request.forward_env is empty")
    if credential_secret is not None:
        environment.extend(
            {
                "name": name,
                "valueFrom": {"secretKeyRef": {"name": credential_secret, "key": name}},
            }
            for name in forward
        )
    return environment
...
def render_action_job(
    ...
    *,
    ...
    if not isinstance(ttl_seconds, int) or isinstance(ttl_seconds, bool) or not 0 <= ttl_seconds <= 604800:
    ...
    image_pull_secrets = [{"name": image_pull_secret}] if image_pull_secret is not None else []
...
def build_parser() -> argparse.ArgumentParser:
    ...
    name = commands.add_parser("name", help="print the deterministic Kubernetes name for a job-record id")
