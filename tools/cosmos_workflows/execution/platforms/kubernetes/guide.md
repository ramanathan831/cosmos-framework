...
The `submit` verb refuses to launch GPU jobs on a cluster with no `nvidia.com/gpu` allocatable. For self-managed clusters, first run the `execution/gpu-host/guide.md` install action on every GPU worker node or bake the same package set into the node image:

```bash
bash execution/gpu-host/scripts/setup-nvidia-gpu-host.sh --backend kubernetes --install --yes
```
...
## What this reference does NOT support (yet)
